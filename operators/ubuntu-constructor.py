...
def read_packages(version, year, file):
    print("--------- " + datetime.datetime.now().strftime("%H:%M:%S") +
          f" || Reading {version}... ---------")
    names = ["Distro-Version", "Distro-Year", "Package", "Architecture", "Version", "Priority", "Section", "Maintainer", "Original-Maintainer", "Installed-Size", "Depends", "Size",
             "Description", "Recommends", "Conflicts", "Suggests", "Build-Essential", "Breaks", "Replaces", "Source", "Provides", "Enhances", "Essential", "Pre-Depends", "Important"]
    with open(file, "r", encoding="utf-8") as f:
        ...
        for package in packages:
            ...
            if row_count % 1000 == 0:
                if row_count == 1000:
                    df.to_csv(f"./distros/ubuntu/{version}/{version}.csv",
    ...
    for version, year, file in distros:
        if df.empty:
            ...
            df = pd.concat([df, pd.read_csv(
                f'./distros/ubuntu/{version}/{version}.csv')], ignore_index=True)
...
distros = [("18.04 - bionic", "2018", "./distros/ubuntu/18.04 - bionic/ubuntu-bionic-packages.txt"),
           ("20.04 - focal", "2020",
            "./distros/ubuntu/20.04 - focal/ubuntu-focal-packages.txt"),
           ("22.04 - jammy", "2022",
            "./distros/ubuntu/22.04 - jammy/ubuntu-jammy-packages.txt"),
